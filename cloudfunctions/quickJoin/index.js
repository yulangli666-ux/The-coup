...
exports.main = async () => {
  ...
  const room = rooms.data.find((item) => !(item.players || []).some((player) => player.openId === openId));
  if (!room) throw new Error("暂无可加入房间");
  const user = await getUser(openId);

  return db.runTransaction(async (transaction) => {
    const latestRes = await transaction.collection("rooms").doc(room._id).get();
    const latest = latestRes.data;
    if (!latest || latest.status !== "waiting") throw new Error("房间已开始");
    ...
    if (!players.some((player) => player.openId === openId)) {
      ...
      addLog(latest, `${user.nickName} 快速加入房间。`);
    }
    await transaction.collection("rooms").doc(room._id).update({
      data: { players, playerCount: players.length, gameLogs: latest.gameLogs, updatedAt: now() }
    });
    await db.collection("users").where({ openId }).update({ data: { activeRoomId: room._id, updatedAt: now() } });
    return { roomId: room._id, roomCode: latest.roomCode };
  });
};
